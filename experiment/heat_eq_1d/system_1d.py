...
def gen_exact_solution():
    ...
    # Save solution:
    np.savez(f"{folder_path}output/heat_eq_data", x=x, t=t, usol=usol)


def gen_testdata():
    """Import and preprocess the dataset with the exact solution."""
    # Load the data:
    data = np.load(f"{folder_path}output/heat_eq_data.npz")
...
x = np.linspace(0, xint, num=100)
t = np.linspace(0, 1, num=100)
xx, tt = np.meshgrid(x, t)
x_int = np.vstack((np.ravel(xx), np.ravel(tt))).T
theta_int = model.predict(x_int)

np.savez(f"{folder_path}output/sup_theta.npz", x_int=x_int, theta=theta_int, ysup=ysup, fl=fl)

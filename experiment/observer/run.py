...
utils.plot_1obs(oo)
# utils.plot_1obs_tf(oo)
# utils.plot_1obs_l2(oo)
...
instants = np.unique(a[:, 4:5])
for lam in lem:
# lam = 5
    def f(t, p):
        a = utils.mu(multi_obs, t)
        e = np.exp(-1*a)
        d = np.inner(p, e)
        f = []
    ...
    sol = integrate.solve_ivp(f, (0, 1), p0, t_eval=instants)
    x = sol.y
    t = sol.t
...
utils.plot_8obs_tf(multi_obs)
utils.plot_8obs_l2(multi_obs)

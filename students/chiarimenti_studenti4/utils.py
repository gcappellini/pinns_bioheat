...
def func(x):
    q = 16
    b = 15
    return (q/(dT))* ((x[:,0:1]**4)/4) + (b/(dT)) * (x[:,0:1] *(x[:,0:1]-1)**2)
...
def pde(x, theta):
    ...
    return a1 * dtheta_tau - dtheta_xx + a2 * 2.3 * theta - a3 * source(x[:, 0:1])
...
def ic_obs(x):
    q = 16
    b = 15
    return (q/(dT))* ((x[:,0:1]**4)/4)



def bc1_obs(x, theta, X):
    dtheta_x = dde.grad.jacobian(theta, x, i=0, j=0)
    return dtheta_x + 1 - k * (x[:,1:2] - theta)
